...
public class Dualconvert extends JFrame {
   ...
   public static String s1="";
    public JPanel largepanel;//panel cotain other panels
...
//                    }
                 ...
                 //--------------
                  ...
                  //Put_value_insteadof_most_and_less=new int[con]; //
                   Put_value_insteadof_most_and_less=new int[100];
                    //check the value in the combo box of most and less array :;--
                    for (int i = 0; i < most_and_less_combobox.length; i++) {
                        if(most_and_less_combobox[i].getSelectedItem().equals(">=")){
                          Put_value_insteadof_most_and_less[i]=0;
                        }
                         else if(most_and_less_combobox[i].getSelectedItem().equals("<=")){
...
/**/
                    ...
                    for(int Com=0;Com<con;Com++){
                        for (int i = 0; i <n; i++) {
                            if(most_and_less_combobox[Com].getSelectedItem().equals(">=")&&maxandmincombobox.getSelectedItem().equals("MAX")){
                   ...
                   for(int Com=0;Com<con;Com++){
                      if(Put_value_insteadof_most_and_less[Com]==0&&maxandmincombobox.getSelectedItem().equals("MAX")){
...
/**/
               //---------------
                ...
                for (int i = 0; i < con; i++) {
                    if(i==con-1){
                        if(maxandmincombobox.getSelectedItem().equals("MIN")){
                            s+="("+new_objectivefunctionarray[i]*-1 + " y"+(i+1)+ ")";
                            System.out.print("("+new_objectivefunctionarray[i]*-1 + " y"+(i+1)+ ")" );}
                        else{
                            s+="("+new_objectivefunctionarray[i] + " y"+(i+1)+ ")";
...
/**/
                ...
                for (int i = 0; i < n; i++) {
                    for (int j = 0; j < con; j++) {
                         if(j==con-1){
                             if(maxandmincombobox.getSelectedItem().equals("MIN")){
                                 s1+="("+new_matrix_value_array[i][j]*-1+ " y"+(j+1)+")";
                                  System.out.print("("+new_matrix_value_array[i][j]*-1+ " y"+(j+1)+")");}
                             else{
                                 s1+="("+new_matrix_value_array[i][j]+ " y"+(j+1)+")";
                    ...
                    if(Put_value_insteadof_most_and_less[i]==0){
                      ...
                      System.out.print(" >= "+ objectivefunctionarray[i]+"\n");
                        ...
                        if(maxandmincombobox.getSelectedItem().equals("MAX")){
                            s1+=" >= "+ objectivefunctionarray[i]+"\n";
                         System.out.print(" >= "+ objectivefunctionarray[i]+"\n");}
                        else{
                            s1+=" <= "+ objectivefunctionarray[i]+"\n";
...
panal_dual.label_s1.setText(s);
panal_dual.txta_s2.setText(s1);
                //----------------------------------------------------------------end check
            }});
    ...
    public static void main(String[] args) {
        Dualconvert ss = new Dualconvert(3, 2);
        ss.setVisible(true);
